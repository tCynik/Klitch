...
- [ ] Профиль пользователя сохраняется и восстанавливается после перезапуска
- [ ] Список каналов отображается, поддерживает добавление / редактирование / удаление
- [ ] PSK генерируется случайно или вводится вручную; невалидный PSK блокирует сохранение
- [ ] `LogicalChannelId` детерминировано вычисляется из PSK (одинаковый PSK → одинаковый ID)
- [ ] DI переключается между Fake и реальными impl без изменения ViewModel
