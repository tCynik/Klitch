...
### Фаза 3 — Геометки: receive path на SQLDelight ✅ Done
...
### Фаза 4 — Чат: data layer на SQLDelight с LogicalChannelId ✅ Done
...
Фаза 1: User & Channels UI          ✅ Done
    │
    ├──→ Фаза 2: Node Provisioning   ✅ Done
    ├──→ Фаза 3: Геометки receive path   ✅ Done
    └──→ Фаза 4: Чат data layer          ✅ Done
```
...
- 2026-04-23: Фаза 3 завершена — IngestReceivedGeoMarksUseCase, GeoMarkRepositoryImpl.persistReceived, DeleteExpiredGeoMarksUseCase (старт + каждый час)
- 2026-04-23: Фаза 4 завершена — ChatMessage.sq, ChatMessageRepository + Impl, IngestReceivedChatMessagesUseCase, MeshToChatAdapter → LogicalChannelId для каналов, ChatRepositoryImpl читает из SQLDelight
