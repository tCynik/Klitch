...
fun MeshIconButton(
    ...
    val contentColor = when {
        !enabled          -> disabledColor   // Disabled: not clickable
        selected == false -> inactiveColor   // Unpressed: toggle is off
        else              -> activeColor     // Enabled: regular or toggle on
    }
    ...
    Box(
        ...
        // Frame — drawn once here, not in individual icons
        ...
        // Icon content — 70% of button size
        Icon(
            imageVector = icon,
...
```kotlin
// Regular button (always enabled colors)
MeshIconButton(
...
// Toggle button (e.g. filter on/off)
MeshIconButton(
...
| Icon | Concept | Key shapes |
|---|---|---|
| Settings | Gear | Circle `r=10` at center + 6–8 teeth as short radial lines or one complex path |
| Chat / Reply | Speech bubble | Rounded rect + tail triangle (bottom-left or bottom-right) |
| Search | Magnifier | Circle + diagonal handle line |
| Menu | Hamburger | Three horizontal lines, equal spacing |
| Close / X | Cross | Two diagonal lines crossing at center |
| Back | Chevron left | `<` shape, centered |
| Add | Plus | `+` shape, centered |
| User / Profile | Person | Circle (head) + arc (shoulders) |
| Notification | Bell | Bell silhouette outline + small horizontal line at bottom |
| WiFi / Mesh | Signal | Concentric arcs from center-bottom |
