...
- **`broadcastIntervalSecs=1800` — это keepalive-анкер**, не основной канал доставки позиции — это делает `sendPosition` (см. `phone-gps-to-radio.md`).
- **`BROADCAST_INTERVAL_HIGH` warning переведён с порога (`>120с`) на проверку несоответствия (`!= LocationConfigDefaults.BROADCAST_INTERVAL_SECS`)** — изначально warning означал "слишком большой интервал", что ломалось намеренным preset'ом 1800с (warning горел всегда, хотя всё настроено правильно). `LocationConfigDefaults.BROADCAST_INTERVAL_SECS` ([LocationConfigModel.kt](app/src/main/java/ru/tcynik/klitch/domain/mesh/model/LocationConfigModel.kt)) — общая константа, читается и `NodeProvisioningUseCase` (что пишет), и `LocationConfigUi` (что проверяет), чтобы не расходились. Warning теперь значит "пока не auto-configured" (firmware default ИЛИ значение изменено снаружи), а не "число больше X".
