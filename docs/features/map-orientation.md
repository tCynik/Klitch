...
- **Scroll (pan)** — отключён: `GestureOptions.PositionLocked` в `MapOptions` (или scroll off при `markToolActive && isCourseUpActive`)
- **Pinch-zoom** — MapLibre под оверлеем; 2+ пальца в оверлее не классифицируются как тап
- **Single-finger** — `Modifier.courseUpMapGestures` в `MainScreen` (см. `CourseUpMapGestures.kt`)
...
### Course-up + добавление геометок

**Статус:** реализовано (`CourseUpMapGestures.kt`, `MainScreen`, `MapLibreLayer`).
...
- **Тапы при метках:** `MarkToolTapDispatcher` (отложенный single / немедленный double) — см. `docs/features/geo-marks.md` → Map gestures.
...
- Геометки (жесты, double-tap, `markToolActive`): `docs/features/geo-marks.md`
...
- `MainScreen` — шесть `LaunchedEffect`: followMe, gesture/followMe-deactivation, resetBearing, restoreZoom, bearing/northLock, courseUp-positioning; course-up overlay
- `CourseUpMapGestures.kt` — `Modifier.courseUpMapGestures` (tap vs Y-zoom, multi-touch)
- `MapLibreLayer` — `isCourseUpActive` → `GestureOptions.PositionLocked`; `markToolActive && isCourseUpActive` → scroll off + жесты в оверлее
...
- План (архив): `docs/archive/course-up.md`
- Предыдущая версия документа (heading-up): `docs/archive/map-orientation.md`
